...
from model_utils import add_model_fc
...
def build_training_model(config):
    ...
    with core.DeviceScope(device_opt):
        ...
        pred = add_model_fc(training_model, config, data, is_test=False)
...
def build_validation_model(config):
    ...
    with core.DeviceScope(device_opt):
        ...
        pred = add_model_fc(validation_model, config, data, is_test=True)
...
def run_main(config):
    ...
    # mini-graph
    ban_fc_graph_mini = net_drawer.GetPydotGraphMinimal(
        validation_model.net.Proto().op,
        "ban_fc_ft_graph_minimal",
        rankdir="TB",
        minimal_dependency=True
    )
    ban_fc_graph_mini.write_svg("ban_fc_ft_graph_mini.svg")
    print("write graph over...")
    sys.exit(0)
    """
